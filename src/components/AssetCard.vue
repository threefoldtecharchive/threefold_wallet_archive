<template>
  <v-card
      ...
      :class='{ blue: balance.asset_code ==="BTC", "white--text": balance.asset_code ==="BTC",}'
  >
    ...
    <v-row>
      ...
      <v-col cols='6' v-if='shouldShowExtra' class='pr-6' align-self='center'>
        <v-btn v-if='shouldShowDeposit'
               color='accent'
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      axios.get(`https://cryptoanchor.io/stellar/deposit?asset_code=BTC&account=${this.id}`, {
      }).then(response => {
    ...
    computed: {
      ...
      shouldShowDeposit() {
        return this.balance.asset_code === 'BTC' && this.balance.balance <= 100;
      },
      shouldShowBuy() {
