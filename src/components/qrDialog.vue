          ...
          <div class='px-2'>
            <v-layout justify-space-between fill-height column px-2>
              ...
              <v-flex>
                ...
                <p class='title mt-0'>{{ formObject.amount | formatBalance }} {{ selectedCurrency }}</p>
              </v-flex>
  ...
  export default {
    ...
    computed: {
      qrText() {
        ...
        return `${this.selectedCurrency}:${this.selectedAccount.id}?amount=${Number(this.formObject.amount).toFixed(7)}&message=${this.formObject.message}&sender=me`;
      },
    },
  };
