                ...
                <v-card-text class="pt-2">
                    <div class="px-2">
                        <v-layout justify-space-between fill-height column px-2>
                            ...
                            <AccountCard no-padding class="my-2" :displayAmount="false" :account="selectedAccount" />
                            <v-flex>
    ...
    export default {
        ...
        computed: {
            qrText() {
                ...
                return `${this.selectedCurrency}:${this.selectedAccount.id}?amount=${Number(
                    this.formObject.amount
                ).toFixed(7)}&message=${encodeURIComponent(this.formObject.message)}&sender=me`;
            },
        },
    };
</script>
