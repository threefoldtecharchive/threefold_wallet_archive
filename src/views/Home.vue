    ...
    <v-layout column fill-height pa-4>
      <div class='content'>
        <draggable v-model='accounts' group='accounts' @start='drag=true' @end='drag=false' delay='400'>
          <AccountCard class='px-1 mb-4' clickable v-for='(account, index) in accounts'
                       :key='`${Home}_${account.name}_${account.balances[0].balance}`' :account='account'
                       @click='seeDetails(account)' humanReadable />
        </draggable>
